...
- **`.services/my.ini` holds absolute paths, so a copied checkout starts a MySQL that dies
  instantly.** It happened: a `.services` tree carried from another machine left `basedir` pointing
  at `C:/Users/User/...`, and mysqld exited before it could write to an error log it could not
  find — the only symptom was `dev-setup.ps1` reporting "did not start listening within 60
  seconds", which reads like a slow machine. `Install-Mysql` now rewrites the file whenever it does
  not name the current checkout, so `pnpm setup:services` fixes it. The data directory itself moves
  fine; only the paths that point at it are wrong.
...
| D-51 | `PAYMENTS_ENABLED=false` is a supported deployment: a complete catalogue with no till. It does not register `POST /api/webhooks/stripe` **at all** and makes the three Stripe keys unnecessary. It exists because the alternative an operator reaches for - the published `.env.production.example` placeholders in a live `.env` - hands over the order pipeline: verification is local HMAC against `whsec_replace_me`, which is in this public repository, so anyone could forge `payment_intent.succeeded` and have orders marked paid, stock decremented and receipts sent. With no checkout to create a PaymentIntent (D-27), every event such a route could receive would be a forgery, so the route should not exist. Default is `true`, so a forgotten key is a loud refusal rather than a silent catalogue. |
| D-52 | Product images are stored in **MinIO on the application's own box** and served by Nginx from the shop's own origin under `/media/`, not from `media.silkgrain.com`. Object storage is not optional - `S3_ENDPOINT`, `S3_BUCKET`, `S3_PUBLIC_URL` and a key pair have no defaults in `env.ts`, so the API does not boot without one - and the owner has no hosted S3 account, so the choice was between a hosted bucket and this. A subdomain would have needed its own A record, its own name on the certificate and its own server block; the same-origin path needs none of the three and makes the CSP strictly tighter, because `img-src 'self'` then covers every photograph and the policy names no external host at all. The arrangement lives in two files that must agree - `S3_PUBLIC_URL` in `shared/.env` and the `proxy_pass` URI, which is the bucket name, in `deploy/nginx/silkgrain.conf` - and nothing checks it: a mismatch is a 404 on every image and no other symptom, so DEPLOY.md §3.5 step five compares them. Nginx allows GET and HEAD only, and MinIO binds to loopback; the bucket is still created and opened for anonymous reads by `Storage.provision` on the first upload, so nothing about the code changes. Two gaps come with it and are recorded rather than hidden: `/srv/minio/data` is in no backup, and `BACKUP_S3_BUCKET` must stay empty because the only S3 endpoint on the box is this one. Owner's call, 2026-08-22. |
| D-53 | **Deactivating a category deactivates its sub-categories, in the same transaction.** `PUBLISHED_PRODUCT` is `products.status = 'active' AND categories.is_active`, and it joins the product's _own_ category — so a child left active under a retired parent is the Phase 3 defect wearing a different hat: `listCategories` drops the child from the tree because its parent is not in it, while the child's products stay in the grid, in search and in the cart, under a heading no menu in the shop leads to. The alternative was to teach the storefront to check the parent too, which means a self-join on `categories` in every product query, every facet and every search — a hot read path made permanently slower to compensate for a rare write. Cascading one write is cheaper, and unlike a read-path fix it is visible in the audit log: one entry for the one action the operator took (D-36), with the children it took named in the note. Reactivating never cascades — a child switched off on its own was switched off for a reason. The two doors into the same incoherence are closed with 409s: a child may not be reactivated while its parent is off, and nothing active may be filed under a deactivated parent. |
| D-55 | Recipes and the FAQ get the same treatment categories did, behind two new permissions, **`content:read` and `content:write`**. Support is granted the read half — answering a ticket means quoting the FAQ back at somebody — and nothing else. Neither is ever deleted: a recipe is a URL somebody may have shared and a FAQ answer is something a customer may have been told, so `is_published = false` is the terminal action, as it is for promo codes, administrators and categories. `recipes.published_at` is written by exactly one route and stamped **once**: taking a recipe down to fix a measurement and putting it back is not publishing it twice, and the storefront's list is sorted by that date. `productIds` is checked before the insert so an id that does not exist is a 409 naming it, rather than a foreign-key 500 — the join exists precisely so "Shop the ingredients" cannot point at nothing. |
| D-56 | **Two empty things are left empty, and that is the answer, not a gap.** Nutrition panels: the only figures in the repository are category reference averages, and `AdminProductInput.nutrition` marks anything it saves `entered`, which asserts that a person checked it against a supplier's certificate (D-20). On a food label that is not a rounding error, so the panels stay absent until the real analysis arrives. Customer reviews, and therefore the home page's testimonial rail: writing them would be inventing customers, and the rail already returns null when it has nothing, so nothing is broken — it draws itself the day a real review arrives. `catalog:fill` prints both reasons when it finishes, so the next person to run it does not go looking for the switch. The About page's portrait tile is a third: it has no field behind it and wants a photograph of the owner's own farms, which a stock landscape would quietly claim to be. |
| D-57 | The brand mark is the header lockup, and `scripts/make-icons.mjs` draws every raster from it — **superseding the half of D-9 that parked the favicon and the OG card "until a real SVG arrives"**. What they were waiting for was a decision about what to draw, and `Wordmark.tsx` answered it in Phase 5: the Phosphor `grains` glyph on a green tile. Everything the script emits is geometry, so no font has to be installed for the icons. The OG card is the one exception and it is sanctioned by the tokens themselves: `type.display` declares `'Cormorant Garamond', Georgia, serif`, so a card set in Georgia is the design's own second choice rather than a substitution invented here — and it is rasterised once and committed, so which serif the generating machine had is settled at that moment and not at request time. `/site.webmanifest` needed its own nginx `location`: 1.24 ships no `.webmanifest` MIME entry, so it served 200 as `application/octet-stream` and Chrome ignored it silently. A `types` block one level up would have made every other file on the site octet-stream, because in nginx `types` replaces the inherited map rather than extending it. |
| D-54 | The category tree is **two levels, enforced rather than described**, and a category is never deleted. `CategoryNode` has always said one level of nesting because the mega-menu is a grid of parents over a chip row of children; a third level would exist in the database and render nowhere, so the writer refuses both doors to it — a category with children may not be given a parent, and a parent that is already somebody's child may not be chosen. There is no DELETE at any role: `products.category_id` is `ON DELETE restrict`, so a delete would be refused for a category anybody had used and allowed for one nobody had — a rule that turns on how much work has been done rather than on what the operator meant — and `parent_id` is `ON DELETE set null`, so it would silently promote a child to the top level. `is_active = false` is the terminal action, as it is for promo codes (D-33's reasoning for administrators). A category's hero image is uploaded through `POST /admin/categories/:id/image` and is not a field in any body: production serves under `img-src 'self' data:` (D-52), so a pasted address stores cleanly, renders as a blank rectangle and reports nothing to anybody. |
