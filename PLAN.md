...
> **Сделано.** Отклонения от плана, все осознанные:
>
> - Пагинация **offset**, а не keyset. Keyset не умеет «страница 3» — а в макете нумерованная пагинация с номерами страниц, — и не умеет сортировку по агрегату `MIN(price)`. При каталоге в десятки товаров offset дешевле и честнее; строка про keyset в задаче 3.2 была написана до того, как макет распарсили.
> - Промокоды разведены на **два эндпоинта**: `/api/cart/validate` не падает из-за кода (истёкший код возвращается в `promoRejected`, корзина всё равно считается), `/api/cart/promo` — кнопка Apply — бросает `PROMO_*`. Решение D-23.
> - Фасеты каталога считаются **каждый без своего фильтра**, а счётчики категорий сворачиваются в верхний уровень, чтобы сайдбар, мега-меню и `GET /api/categories` не показывали для одной категории разные числа. Решение D-21.
> - Тарифы доставки не отдельным эндпоинтом, а внутри `CartQuote.shippingOptions`: корзине и чекауту нужны и опции, и пересчитанная под эту корзину цена — два запроса вместо одного ничего не дают.
> - `optionalCustomer` — новый guard в `plugins/auth.ts`: гостю корзина доступна, но лимит «один раз на клиента» проверяется, если токен есть. Прав не даёт, просроченный токен не ошибка.
> - Найден и исправлен дефект в контракте: `z.coerce.boolean()` превращал `?inStock=false` в `true`, потому что это `Boolean("false")`.
> - Тестов **52** (26 каталог + 26 корзина), всего по api стало 93. Фикстура своя, а не сид — решение D-24.
> - Сверх плана: прогон всех эндпоинтов по **реальным сидам** (32 товара), потому что фикстура не содержит товаров без нутриентов, сертификатов и отзывов, а схема ответа работает сериализатором — расхождение это 500 в рантайме, а не ошибка типов.
>
> **Что нашло состязательное ревью и что исправлено.** Шесть независимых проходов по диффу, каждая находка проверялась тремя скептиками; из 52 заявленных выжили 20.
>
> - **Фасеты были только по категориям и цене.** В сайдбаре макета пять карточек, и список весов на клиенте захардкодить нельзя: в макете 1/2/5/10/25/50 lb, а в этом каталоге веса другие. Добавлены фасеты `weights`, `origins`, `certifications`, каждый считается без своего фильтра.
> - **Отключённая категория пряталась из меню, но её товары оставались** в сетке, поиске, на своей странице и в корзине — худший из вариантов. `categories.is_active` теперь часть базового предиката каталога.
> - **Промокоды перебирались 240 попыток в минуту** с одного адреса: `@fastify/rate-limit` даёт маршруту со своим `config.rateLimit` отдельную корзину, а не добавляет к глобальной. У `/cart/promo` теперь 12 попыток за 5 минут. Остаток по `/cart/validate` требует лимита, видящего тело запроса, — в `BACKLOG.md`.
> - **Прогресс-бар спорил сам с собой**: с промокодом на бесплатную доставку строка доставки была нулевой, а бар писал «до бесплатной доставки $69».
> - **`/api/search/suggest` требовал `q`**, хотя чипсы «популярное» показываются как раз до ввода. Теперь пустой запрос валиден и возвращает только чипсы.
> - Массивы фильтров были без верхней границы; скидка могла уйти в минус при отрицательном `max_discount_cents` (в колонке нет CHECK).
> - **Найдено попутно: у локального MySQL не было `ONLY_FULL_GROUP_BY`**, хотя стоковый MySQL 8 включает его по умолчанию — то есть сгруппированный запрос мог пройти локально и упасть в проде. Режим добавлен во все три места, где он объявлен, сервер перезапущен, все тесты зелёные уже под ним.
> - Закрыты дыры в тестах: черновик через корзину, ставка налога действительно из БД (в фикстуре она совпадала с константой-фолбэком), скидка на границе бесплатной доставки, повторяющиеся query-параметры, поиск по blurb и subtitle, товар без нутриентов и отзывов, «та же категория первой» в related, порядок вариантов по position. Тестов стало **107**.
...
> **В работе, не закончена.** Сделано и покрыто тестами: 4.1 (номера заказов), 4.3 (вебхук Stripe), 4.4 (транзакция «оплачено»), 4.8 (чтение заказа). Итого 39 тестов.
>
> - **Вторая половина критерия приёмки уже выполнена:** повторная доставка события не создаёт дубля, и это доказано — плюс гонка двух одновременных доставок, и ретрай события, чья первая попытка упала. Проверка подписи Stripe — чистый HMAC по байтам запроса, поэтому весь контур вебхука тестируется без аккаунта: подписи в тестах генерирует сам SDK.
> - **Первая половина заблокирована:** четыре сценария с тестовыми картами требуют настоящего `sk_test_`, а в `.env` лежит заглушка из `.env.example`. Решение D-27: адаптер, который ни разу не вызывался, по правилу «никаких заглушек» не пишется — 4.2 и 4.6 просто не начаты.
> - **PayPal (4.5) вынесен в `BACKLOG.md`** решением владельца, D-26.
> - Отклонение от плана: номера заказов без таблицы-счётчика. `MAX()` по колонке фиксированной ширины точен, а UNIQUE-индекс с ретраем решает гонку — счётчик же сделал бы строку, на которой сериализуется каждый заказ, и разъехался бы с реальностью при первой ручной вставке.
> - Найдено тестами: статус ошибки, правильный для покупателя, неправильный для Stripe. `INSUFFICIENT_STOCK` — это 422 для того, кто набирает корзину, а 4xx в ответ Stripe означает «запрос кривой», хотя запрос был в порядке и упала работа. Провал обработчика теперь 500.
> - **4.7 сделано.** BullMQ между вебхуком и SMTP: вебхук обязан ответить Stripe за секунды, а разговор с медленным почтовым провайдером превратил бы успешный платёж в редоставку уже сделанной работы. Воркер перечитывает заказ через ту же проекцию, что отдаёт страница заказа, — чек и страница не могут показать разные суммы. От второго письма защищают две вещи: транзакция сообщает, изменила ли она что-нибудь, и id задачи — это номер заказа, который BullMQ второй раз не примет. В тестах ничего не подменено: Mailpit — настоящий SMTP-сервер с HTTP API, и «письмо ушло» проверяется опросом того сервера, который его принял.
> - Ещё два дефекта, которые видно только при реальном запуске: `latest_charge` у Stripe объявлен как `string | Charge | null`, но на платеже без списания его просто нет — разыменование давало 500 и цикл редоставок; и BullMQ строит свои ключи через `:`, поэтому кастомный id с двоеточием отвергался, и каждая постановка в очередь молча падала в лог.
> - Без ключей в фазе не осталось ничего: всё оставшееся (4.2, 4.6) обращается к API Stripe.
...
**8.1 — покрытие.** Пороги записаны в `vitest.config.ts` двух пакетов и падают вместе с прогоном,
а не живут в отчёте, который никто не открывает: api — 95 % строк / 82 % ветвей, contracts —
100 % на `money.ts`, `pagination.ts`, `rbac.ts`. Три файла подняты до 100 % новыми тестами
(`pagination.test.ts`, `rbac.test.ts` и два охранных теста на `Money`). Скрипт — `test:coverage`,
и именно он стоит в `verify`, а не `test`: порог, который гейт не запускает, — это не порог.

**8.2 — Playwright.** Двадцать проверок (шесть сценариев × два профиля — Desktop Chrome и
Pixel 7, потому что мобильная навигация по D-19 — отдельный компонент, а не сузившийся тот же).
Гоняются против `preview`, то есть против собранного артефакта, а не против `dev`: dev-сервер
отдаёт другой граф модулей, и зелёный прогон по нему ничего не говорит о том, что скачает
посетитель. Проверяются швы, которых не видит ни один из 435 интеграционных тестов: корзина,
пережившая полную перезагрузку страницы, с той же ценой, что показывала карточка товара;
сессия, пережившая перезагрузку, — и рядом доказательство, что она пережила её через httpOnly
cookie, а не через `localStorage` (D-15); цифра бесплатной доставки на витрине, сверенная с тем,
что вернул `GET /api/settings` (D-22); настоящая 404 со своим текстом. E2E-слой не принадлежит
ни одному пакету, поэтому у него собственный `tsconfig.json` в корне — иначе type-aware ESLint
не видит эти файлы вовсе.

Чего здесь нет и почему: **платёжных путей**. Задача просит «оба платёжных провайдера и отказ
оплаты». PayPal вынесен в `BACKLOG.md` владельцем (D-26), а `POST /api/checkout/intent` не
написан, потому что для него нет ключа Stripe (D-27). Тест, который подменил бы провайдера
моком, проверял бы мок. Пробел записан, а не закрашен.

**8.4 — бюджет бандла.** `scripts/bundle-budget.mjs` считает входной чанк и всё, что тот тянет
статически, и падает при превышении: web 158.6 KB при лимите 250, admin 164.9 при 320. Динамический
`import()` не считается — разбиение по роутам именно для того и сделано. Стоит в `verify`.

**8.7 — безопасность.** Восемь независимых проходов по API, каждая находка отдана трём скептикам с
инструкцией её опровергнуть: достижимость, эксплуатируемость, «уже закрыто чем-то другим».
Выживала та, которую опровергли меньше двух из трёх. Ни обхода авторизации, ни инъекции, ни
нарушения шести непререкаемых правил: денежный путь, контур вебхука, матрица прав и схема
refresh-токенов выдержали чтение построчно.

Что проверено и оказалось в порядке — список не менее ценный, чем список дыр: `sql.raw` и
конкатенации нет ни в одном из 65 мест с `sql``​``; все девять пользовательских `LIKE`идут через`likePattern`, экранирующий `\`, `%` и `_`; сортировка — `switch` по Zod-enum, динамических имён
колонок нет, `multipleStatements: false`; подпись Stripe сверяется постоянным временем с допуском
300 с, `event_id` занимается под уникальным индексом до любой работы; расчёт заказа идёт под
`FOR UPDATE`, остаток списывается `UPDATE ... WHERE stock_qty >= qty` с откатом; писателя
`status = 'paid'` вне `settle.service.ts` не существует (D-28 держится); все 40 админских
маршрутов под `requirePermission`, и обход читает таблицу маршрутизации, а не список рядом;
refresh-токен — 48 случайных байт как HMAC-дайджест, ротация под блокировкой, повтор убивает
семью; `equaliseTiming` жжёт настоящую проверку на неизвестном адресе, так что оракула
перечисления нет ни на одном входе; загрузка перекодируется в webp с принудительным content-type
и ключом-хешем, что закрывает XSS, обход пути и перезапись по построению.

**Пять находок выжили**, и все пять исправлены — см. D-38…D-41 и коммит 8.7 part two. Самая
серьёзная не была уязвимостью: `*.sql` в `.gitignore`, написанный для дампов, прятал все четыре
миграции, тогда как их журнал и снапшоты лежали в репозитории. Свежий клон получал манифест на
четыре несуществующих файла и не мог ни мигрировать, ни прогнать тесты API — то есть `pnpm verify`
проходил ровно на одной машине в мире.

Три находки ушли владельцу как Q-48…Q-50, потому что каждая — решение, а не правка: лимиты
промокода принадлежат транзакции чекаута, которой пока нет; число хопов доверенного прокси должно
совпасть с тем, что напишет Nginx в Фазе 9; дедупликация изображений по хешу содержимого — это
поведение, которое стоит подтвердить, а не выбрать за владельца.

**gitleaks заменён своим сканером** — см. `scripts/secret-scan.mjs` и раздел про 8.7 в `STATE.md`.
