...
> **Сделано.** Отклонения от плана, все осознанные:
>
> - База — **MySQL 8.0.42 на 3307**, а не MariaDB 10.4 (решение D-6, принято ещё в Фазе 0). Поэтому `CHECK`-констрейнты реально работают: на MariaDB 10.4 они бы молча ничего не проверяли.
> - Таблиц **32**, а не 25: сверх ТЗ добавлены `product_badges`, `promo_redemptions`, `recipe_products`, `shipping_rates`, `settings`, `faqs`, `contact_messages`, `audit_log`. Таблиц `carts`/`cart_items` нет — решение D-18.
> - Откат — не `down`-миграции, а `pnpm db:reset` (решение D-17, вопрос Q-44).
> - `related_product_ids (JSON)` из ТЗ заменён на join-таблицу `recipe_products`: JSON-массив не умеет ссылаться на удалённый товар.
> - Сидов **32 товара** (16 из макета + 16 дописанных), 79 вариантов, 147 отзывов, 7 демо-заказов с пересчитанными по-настоящему суммами, 5 оптовых заявок, 6 рецептов с написанными текстами.
> - Тестов **41** вместо запланированного набора по auth: сверх auth покрыты формат ошибок, `.strict()` на входе, заголовки helmet, OpenAPI-документ и валидация `env`.
...
| 5.11 | Адаптив по спецификации `for adaptive/README.md`: два брейкпоинта (1024 / 760), мобильная панель навигации 330px, гамбургер, 44px тач-таргеты, 16px в полях. Классы `tablet:`/`mobile:` пишутся вместе с разметкой каждого экрана, отдельного прохода нет | 10  |
| 5.12 | A11y-проход: клавиатура во всех модалках, focus trap, Esc, skip-link, aria-live на корзину, alt'ы                                                                                                                                                         | 5   |

**Acceptance:** каждый экран сверен с макетом по цветам/отступам/типографике; Lighthouse **на десктопе и на мобиле** ≥ 90 Performance, ≥ 95 A11y и SEO; клавиатурная навигация и фокус-ловушки работают; каждый экран проверен на 1440, 1024 и 760.

> **Решение D-19 (владелец, 2026-07-29): адаптив делаем сразу, внутри фазы.** Задача 5.11 возвращена, оценка фазы остаётся **70–90 ч**. Смысл в том, что 5.11 — это не отдельный проход в конце, а строка бюджета: `tablet:`/`mobile:` классы пишутся в разметку по ходу каждого экрана. Отложить и прошить потом стоило бы 30–40 ч. Пункт про адаптив убран из `BACKLOG.md`, D-8 отменено.
...
| 7.3 | Products: список с поиском/фильтрами + форма создания/редактирования (варианты, сертификаты, SEO, статус) + **полная панель Nutrition Facts** — размер порции, порций в упаковке, калории, жиры, насыщенные, углеводы, сахара, клетчатка, белок, натрий, состав, аллергены; ввод в г/мг, хранение в целых мг. Плюс миграция: признак «заполнено вручную» против «справочное значение из сида» (решение D-20, Q-43) | 12  |
